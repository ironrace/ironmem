#!/usr/bin/env python3
"""Generate/check Claude and Codex MCP wrapper scripts from one template."""
...
TARGETS = [
    ROOT / ".claude-plugin" / "bin" / "ironmem-mcp.sh",
    ROOT / ".codex-plugin" / "bin" / "ironmem-mcp.sh",
]

TEMPLATE = """#!/usr/bin/env bash
# Generated by scripts/sync_mcp_wrappers.py; do not edit by hand.
set -euo pipefail
...
# Version consistency check: block mismatches so plugin metadata and binary
# capabilities cannot drift silently.
...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true", help="fail if wrappers differ from template")
    args = parser.parse_args()

    if args.check:
        drifted = [path for path in TARGETS if path.read_text() != TEMPLATE]
        if drifted:
            for path in drifted:
                print(f"wrapper drifted: {path.relative_to(ROOT)}", file=sys.stderr)
            print("Run: python3 scripts/sync_mcp_wrappers.py", file=sys.stderr)
            return 1
        print("MCP wrappers match template")
        return 0
