...
3. Parse the verified plan file's `### Task N:` headings into
   ...
   the limit.
4. Run `git rev-parse HEAD` and substitute the full 40-character sha it
   prints for both `base_sha` and `head_sha` below — do not write the
   literal string `HEAD`. The server refuses `head_sha` unless it is 7-64
   hex characters: a revision expression is not a fixed commit, so the
   session would have nothing to detect drift against. Build the manifest
   `{plan_hash: final_plan_hash, base_sha:<sha>, head_sha:<sha>,
   plan_file_path:<path>, tasks:[...]}`. Add
