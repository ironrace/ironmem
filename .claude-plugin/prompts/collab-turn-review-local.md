...
4. In `review_local=full`, run `/ultrareview-local --report-only` auditing
   ...
   pushes, a tree already carrying those edits. With the flag no fix agent is
   dispatched at all, and any review lens that runs commands which write is
   given its own throwaway worktree at the review snapshot, removed when it
   returns — `ROSTER.mutates` in `ultrareview.js` declares which lenses those
   are and is the only place that may say so, here included. Independently
   verify the synthesized findings and keep only confirmed
...
6. Group confirmed findings into non-overlapping fix clusters. These fix
   ...
   review head and dispatch fix subagents in parallel. Give each subagent
   exactly one cluster, tell it not to touch
