...
2. Call `mcp__ironmem__ironmem_collab_start` with those four fields.
...
4. Enter Plan Mode and draft your first plan for `<task>` — the draft is
   ...
   Plan Mode, call `mcp__ironmem__ironmem_collab_send` with
...
3. Call `mcp__ironmem__ironmem_collab_status` to read `task`,
...
1. `mcp__ironmem__ironmem_collab_wait_my_turn` with
...
4. **`is_my_turn == true` → STOP POLLING.** Do not call `wait_my_turn`
   ...
   to step 5.
5. `mcp__ironmem__ironmem_collab_status` → read `phase`,
   `current_owner`, `review_round`.
6. `mcp__ironmem__ironmem_collab_recv` with `receiver="claude"`.
   Ack each message via `mcp__ironmem__ironmem_collab_ack`. An
...
- **Never** call `mcp__ironmem__ironmem_collab_end`. It is
