...
`$BRANCH`, `$TOPIC`, `$ARTIFACT_REF`, `$MODE`) → ingests ONLY the worker's
...
a drawer/file and returns `{ref, hash, ≤3-line summary}`; the orchestrator
surfaces ONLY ref+hash+summary for approval (never the full body); a
`collab-turn-submit.md` worker sends the approved artifact by ref.
...
| `PlanClaudeFinalizePending` | **Enter Plan Mode and get user approval — this is the v1 commit-point gate.** Under reference-only gates, dispatch `collab-turn-plan-finalize.md` (planning/opus), which produces the final plan (incorporating Codex's review notes unless they conflict with user intent), writes it to a drawer as `{"plan":...}`, and returns `{drawer_id, hash, ≤3-line summary}`; surface ONLY ref+hash+summary for approval. On approval dispatch `collab-turn-submit.md` (mechanical/sonnet) with `$TOPIC=final` `$ARTIFACT_REF=<drawer_id>` to send `topic="final"` (v1 `final` is the only v1 topic wrapped in JSON). After send, `PlanLocked` is reached. Ingest only the ≤3-line verdict; loop. |
...
`$MODE=submit` to derive + validate the manifest and send `task_list`. Only
refs/paths cross the orchestrator boundary.
...
the `$MODE=compose` worker authors the markdown plan via `writing-plans` and
its own "execute now?" handoff is the user gate.
...
3. **Submit.** On approval, dispatch `collab-turn-task-list.md` again with
   `$MODE=submit`. The worker parses each `### Task N:` heading into
   `{id, title, acceptance:[...]}`, builds the `task_list` manifest
   ...
   eligibility rule in `docs/COLLAB.md` holds), and `collab_send`s
   `topic="task_list"`. If zero tasks parse, the worker sends a
