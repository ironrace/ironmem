...
server it is connected to. Do not re-derive it from `generation` or
`handoff_pending`. The verdict is per server: it is also the dispatcher's
verdict only when both harnesses run `serve --connect <same socket>`, which
`ironmem doctor` confirms per harness. When it is `false`,
**select nothing**: do not call `collab_wait_my_turn`, do not touch a
role, and do not open a prompt. Report the lockout — session id, phase,
`codex_generation` — with the one remedy the same read admits, in this
order (`reclaimable`, `idle_secs`, and `handoff_pending` sit in
`codex_lease`; `ended_at` and `dead_session_secs` are top-level keys of the
same response): `ended_at` not null → the session is sealed, nothing to
remedy; `reclaimable: true` → `force_reissue: true` on `session_handoff`
for `agent: "codex"`, claimed through this daemon by the dispatcher;
`handoff_pending: true` → a minted token nobody claimed, which a repeat
`force_reissue` on a dead session echoes for the dispatcher to claim;
`idle_secs >= dead_session_secs` and nobody means to finish → `collab_end`
with `abandon: true` (a plain `collab_end` without `abandon` is lease-gated
like every other call this shim makes, and refused outright in the coding
phases); otherwise the remaining wait. Then exit. When it is `true`,
continue — the same `collab_status` read serves everything below.
