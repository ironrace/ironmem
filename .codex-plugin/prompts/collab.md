...
7. Branch on `phase`:
   ...
   - **`CodeReviewFixGlobalPending`** → Codex's only mandatory v3 coding
     ...
     order this phase runs FIRST (before Claude's `review_local` audit).
     Run the global review action below.
   - **`CodeReviewLocalPending`** → Claude's audit turn — Claude is
     running full or reduced `review_local` against your
     `review_fix_global` commits. Exit.
...
| `PlanCodexReviewPending` | In the message set already returned by the per-invocation `collab_recv`, locate the message whose `topic="canonical"`. For a current row, take its `drawer_id` and call `get_drawer` with that id to obtain the canonical plan before reviewing. The default receive is compact; `full:true` is compatibility-only. Legacy rows without a `drawer_id` may include inline `content`. Use `collab_status(verbose:true)` only as a legacy/status fallback when the compact message-reference path cannot be used. Call `collab_send` with `sender="codex"`, `topic="review"`, `content=<JSON {"verdict":"...","notes":["..."]}>`. Allowed verdicts: `approve`, `approve_with_minor_edits`, `request_changes`. Shortcut: if verdict is exactly `approve`, you may call `collab_approve` with `agent="codex"`, `content_hash=<canonical_plan_hash from collab_status>` instead. **Review cap (server-enforced):** you have exactly one plan-review pass (`MAX_REVIEW_ROUNDS = 1` at `crates/ironmem/src/collab/state_machine/mod.rs:28`). After this review the server always advances to `PlanClaudeFinalizePending`, including when your verdict is `request_changes`; there is no return to synthesis. Put every requested edit, split, risk, and 20-minute task-sizing concern in this one response so Claude can fold it into the final Superpowers task plan. |
...
editing. If the newest checkpoint is `batch_complete`, first try to reuse
its gate proof: require clean pushed-head proof, local
`HEAD == checkpoint.head_sha`, `checkpoint.gates_sha == checkpoint.head_sha`,
`checkpoint.gates_result` starts with `passed`, and
`checkpoint.gates_commands` exactly matches the current required gate set.
When all checks hold, send `implementation_done` without rerunning gates.
Rerun gates only on HEAD drift, changed gate commands, failed pushed-head
proof, or a checkpoint that lacks the new gate-proof fields. Do not rerun
completed tasks.
...
7. Exit. The session is now `CodeReviewFixGlobalPending` with Codex as
   ...
   `review_local` audit runs after that at `CodeReviewLocalPending`.
