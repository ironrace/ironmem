...
fn seed_db(dir: &Path, session_id: &str, phase: &str, owner: &str, review_round: i64) -> std::path::PathBuf {
    ...
    conn.execute(
        "INSERT INTO collab_sessions
            (id, phase, current_owner, implementer, task_review_round, global_review_round, last_head_sha, pr_url)
         VALUES (?1, ?2, ?3, 'claude', 0, ?4, 'abc123', NULL)",
        rusqlite::params![session_id, phase, owner, review_round],
    )
    .unwrap();
    db
}
...
fn reads_existing_session_row() {
    ...
    let db = seed_db(dir.path(), "sess-1", "CodeReviewFixGlobalPending", "codex", 2);
...
fn missing_session_is_an_error() {
    let dir = tempfile::tempdir().unwrap();
    let db = seed_db(dir.path(), "sess-1", "PlanParallelDrafts", "claude", 0);
    let err = read_session_state(&db, "nope").unwrap_err();
    assert!(err.to_string().contains("nope"), "error names the missing session: {err}");
}

#[test]
fn terminal_phases_detected() {
    let dir = tempfile::tempdir().unwrap();
    let db = seed_db(dir.path(), "done", "CodingComplete", "claude", 1);
