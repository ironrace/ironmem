...
No current launch is authorized. A future static helper must validate a supervisor-built, versioned exact minimal `envp`, reject every unknown or secret-shaped key without exemption, and emit bounded value-free reason codes only; credentials may not use environment variables. The privileged supervisor may prepare/measure but never exec HumanLayer, broker, or provider as root: before HumanLayer fd-exec it clears supplementary groups, sets all real/effective/saved GID then all real/effective/saved UID to dedicated pilot identities, clears/locks capabilities and securebits, sets `no_new_privs`, applies approved resource/profile restrictions, closes unapproved FDs, verifies post-drop state in child context, and binds it to the nonce. A future single supervisor-controlled transaction must bind helper validation to the exact immutable `envp`, nonce, Linux mount namespace, read-only workspace policy files, canonical repository/policy descriptors, exact invocation policy, post-drop attributes, and same-namespace HumanLayer descriptor execution. A separate provider requires a broker that HumanLayer alone can invoke and that open-fd executes the approved artifact under those same unprivileged restrictions; an embedded provider requires proof of no separate spawn path. `SUPERVISOR-ARTIFACT-MISSING`, `STATIC-HELPER-MISSING`, `INVOCATION-POLICY-MISSING`, `LAUNCH-TRANSACTION-MISSING`, `CREDENTIAL-MECHANISM-MISSING`, and `PROVIDER-SPAWN-MEDIATION-MISSING` are launch blockers until independently implemented, reviewed, pinned, and approved in separate additive-code issues.

The **Host operator** owns supervisor installation, irreversible drop, post-drop/transaction evidence, and stop/remediation. The **Security reviewer** approves supervisor, invocation-policy, transaction schema/manifest, and credential-contract changes and resumption. The **Model-provider administrator** owns provider/broker invocation, account, and non-environment authentication evidence for `CREDENTIAL-MECHANISM-MISSING`. These roles cannot waive a blocker.
...
| `SUPERVISOR-ARTIFACT-MISSING`, `STATIC-HELPER-MISSING`, `INVOCATION-POLICY-MISSING`, `LAUNCH-TRANSACTION-MISSING`, `CREDENTIAL-MECHANISM-MISSING`, or `PROVIDER-SPAWN-MEDIATION-MISSING`/embedded-no-spawn proof is missing | Abort; public/synthetic remains eligible only, not authorized, until the authoritative threat-model gate is satisfied. |
...
| Launch-transaction future-contract test | A separately implemented supervisor proves nonce/envp/namespace/file/artifact/invocation/post-drop binding and same-namespace descriptor execution with no intervening agent window. |
...
Public or synthetic code on a disposable repository and dedicated host identity is eligible only after every operational gate in the authoritative threat model passes; no current pilot can proceed. This includes independent resolution of `SUPERVISOR-ARTIFACT-MISSING`, `STATIC-HELPER-MISSING`, `INVOCATION-POLICY-MISSING`, `LAUNCH-TRANSACTION-MISSING`, `CREDENTIAL-MECHANISM-MISSING`, and provider mediation or embedded/no-spawn proof. This preserves the intended credential, confidentiality, and GitHub blast-radius limits while leaving future orchestration evaluation conditional on independently implemented controls.
