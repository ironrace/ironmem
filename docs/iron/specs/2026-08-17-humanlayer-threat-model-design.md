...
| Helper runs before the frozen view/open descriptors exist; helper receives/reopens credential or readable audit FD, `/proc` FD, extra/substituted FD, or content echo; planned/post-drop record mismatch; LSM label/domain/transition/policy drift; or monitor-mode/config/key/socket/ledger/privilege, authorization signature/key/staleness/replay/IPC/FD/binding/race, crash/restart/reboot/ledger/TPM/counter/epoch/clock failure | Abort before exec; preserve only bounded value-free evidence, never credential/audit content; invalidate outstanding authorizations and require counter/ledger reconciliation; restore/review the affected transaction, LSM, monitor, or broker contract before Security reviewer-approved resumption. |
...
| Broker-authorization future-contract test | Supervisor-mode monitor config/public-key/socket/ledger/protocol/UID/profile drift or overprivilege fails; forged/wrong-key/stale/replayed/counter-rollback/unavailable-monitor/IPC/sealed-FD/wrong-binding/consumption-race authorization fails; crash before/after durable commit/ack, restart/reboot, ledger corruption/deletion, snapshot rollback, TPM reset/replacement, counter ahead/behind, epoch/PCR, and monotonic-clock anomalies fail closed; monitor permits exactly one descriptor-bound broker use. |
