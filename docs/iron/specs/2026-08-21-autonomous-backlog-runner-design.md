...
**Status:** **Approved** — 2026-08-23, by Jeff Crum, at rev 4. Rev-5 amendment approved 2026-08-24. Rev 6 is validation-only (rung 0 of the `/goal` build ladder) and records measurement, not a design change requiring re-approval. Rev 7 records implementation findings from rungs 7-8 and closes two open questions; rev 8 records rung 9, which builds open question 9's residual; rev 9 records rung 10, which executes the data flow's post-green arc that rungs 5 and 6 built the pieces of; rev 10 records rung 11, which executes the last unexecuted arrow in that same data flow. None changes an approved decision.
**Amendment approved and merged 2026-09-15 (at its rev 3):** `2026-09-12-autopilot-decomposition-and-merge-authority-design.md` proposes dispatching against sub-issues rather than whole issues, adds a work-shape axis, and proposes merge authority. **It reopens three decisions this document and the code treat as settled** — the `iron-build` non-goal (line 52), rung 6's *"cannot become one"* (`merge.rs:59-63`), and rung 11's remediation bound. ⟨r12⟩ That amendment is now approved, so those three decisions are reopened by it; this document is superseded where the two disagree, and its Decision 4 adds a fourth prerequisite — `gate_green` is false for ever once a human commits to an autopilot branch.
**Revision:** rev 12 — ⟨r12⟩ records the **reviewer harness swap**: Codex's subscription ended on 2026-09-15, so the Reviewer is routed to **Muse**. The routing's stated reason (cross-model with respect to a Claude IC) survives; one guarantee does not — Codex's `--output-schema` forced the verdict's shape and Muse has no equivalent, so the shape is stated in the prompt and anything else is refused as `NoVerdict`. Two properties were measured rather than assumed: `--approval-mode never` does not deny tool calls, and no Muse flag makes the workspace read-only to the shell, so a reviewer that modifies the checkout it is judging now loses its verdict. No architectural change: the reviewer still decides, and `decide_merge` is still the single answer to "may this merge?". Earlier: rev 11 — ⟨r11⟩ records **defect 7, the pull request nobody opened**: rev 1's *"push feature branch, open PR"* was one arrow, and only its first half was ever built. The IC's goal condition now requires the **push** (a gate runs in the worktree, so "the gate passes" was truthfully satisfiable with an empty remote branch), and `advance` opens the pull request itself via `gh pr create`, guarded by a `--state all` read so a PR a human closed is never re-opened. No architectural change: the deny-list is about pushes and `gh pr create` is an API call, the reviewer still decides, and `decide_merge` is still the single answer to "may this merge?". Earlier: rev 10 — ⟨r10⟩ records rung 11, **the red path**: the data flow's *"NEEDS CHANGES → re-dispatch IC to fix"* arrow is now *executed*, by `autopilot advance --remediate` over `autopilot/remediate.rs`. Rung 10 executed the `PASS` side of the reviewer's fork and recorded the other side as its residual; this closes it, under the attempt cap the error table already named. No architectural change: the reviewer still decides, `decide_merge` is still the single answer to "may this merge?", and an exhausted issue still ends with the PR open and a human told. Earlier: rev 9 — ⟨r9⟩ records rung 10, **the loop closing**: the arc this document's data flow draws from a green IC to a merged PR is now *executed*, by `autopilot advance` (`autopilot/advance.rs`). Rungs 5 and 6 built reviewing and merging; nothing joined them, so `queue::DeferReason::AlreadySucceeded` was a permanent dead end and a green PR waited on a human. No architectural change: `advance` decides only which issue is at which step, and `decide_merge` is still the single answer to "may this merge?". Earlier: rev 8 — ⟨r8⟩ records rung 9: open question 9's stated residual, the three judgment-shaped one-shot calls, is **built** (`autopilot/advise.rs`), off by default, and structured so the loop does not depend on it; the escalation a supervisor raises is now reported to a human on the issue rather than only in a drawer. No architectural change. Earlier: rev 7 — ⟨r7⟩ folds in the build ladder's own findings: open question 9 (is the Lead a Claude session?) is **closed** by rungs 7 and 8, and open question 7's wall-clock half is closed by rung 7. No architectural change; the two-tier, four-role shape is unchanged. Earlier: rev 2 incorporated two review passes (⟨r2⟩); rev 3 folded in the first validation round (⟨r3⟩); rev 4 folded in a controlled second round (⟨r4⟩) that reversed one rev-3 finding, retired one blocker, and settled the IC primitive; rev 5 (⟨r5⟩) **generalises that primitive by one parameter** and adds model routing; rev 6 (⟨r6⟩) is rung 0 of the build ladder — it measures the ⟨r5-doc⟩ claims for real, closes open question 6a with a working mitigation, and writes the turn-prompt template rev 4/5 never tested. See *Open questions*.
...
**Also unchanged from rev 4:** `--name` gives ICs deterministic addresses (`ic-ironmem-283`) for both `ListAgents` and the abort path.

⟨r12⟩ **The verdict is no longer schema-validated, and that is a real loss, not a rewording.** Rev 4 relied on Codex's `--output-schema` to force the Reviewer's reply into a shape; `muse exec` has no equivalent. The shape is now stated in the prompt and `parse_review_message` refuses anything else, so the failure is fail-closed — a non-conforming reply is `NoVerdict`, which holds the PR — but it is enforced by a *model following an instruction* rather than by the harness. Two further properties were measured on Muse Code 1.3.0 and are worth recording because they were assumed rather than checked at first: `--approval-mode never` does **not** deny tool calls (a reviewer denied its tools would answer from the prompt alone and still report `completed`), and **no Muse flag makes the workspace read-only to the shell** — `--disable-write` covers only the non-shell write tools, and a live run wrote to the checkout unprompted. The read-only property rev 2 names is therefore enforced *after* the fact, by comparing the checkout's porcelain status **and its `HEAD`** across the run and discarding the verdict of any reviewer that modified what it was judging — both halves, because a reviewer that *commits* what it wrote leaves the porcelain output it started from while moving the branch a merge would take. **That residual is now closed.** `codex exec -s read-only` sandboxed the network too, and `muse exec` under `--approval-mode never` does not — a checkout comparison cannot see a remote action, so for a time nothing but the prompt's wording stopped a reviewer running `git push`, `gh pr merge` or `gh pr comment`, the actions this document reserves for `merge.rs` and the human. `--sandbox-network restricted` is now passed, measured in a live tool-calling run: `git status --porcelain` still exits 0 while `curl https://example.com` exits 6 and `git ls-remote https://github.com/...` exits 128, both "Could not resolve host". The refusal arrives as **DNS failure, not a typed denial**, so nothing should expect a permission message; and because `gh pr diff` cannot work either, the prompt now names only the local `git diff`.
...
- ⟨r11⟩ **The IC pushes; the Lead opens the PR.** Rev 1 read "open PR" as part of the IC's terminal state and nothing ever built it — see *The pull request nobody opened* below. `gh pr create` is a GitHub API call, so this does not touch the deny-list, which is about pushes.
...
2. **The review trigger is "no review has read *this* commit on *this* base", not "no review exists"** — the same two comparisons rung 6 enforces before merging. An IC that pushed a fix is re-reviewed automatically, and so is a PR somebody retargeted; a PR whose head and base have not moved is never re-billed. Head alone was not enough: a retargeted PR's head never moves, so it reported as already reviewed on every pass and sat at `BaseBranchMismatch` with no way out.
...
### The red path — rung 11 ⟨r10⟩

Rev 9 recorded rung 10's residual: the data flow's *"NEEDS CHANGES → re-dispatch IC to fix"* arrow was drawn and never run, so a held PR was reviewed, commented and labeled exactly as rungs 5 and 6 specify, and then waited for a human. `autopilot/remediate.rs` executes it, under `autopilot advance --remediate`, and the error table's own bound — *"counting against the same per-issue attempt cap"* — is what makes it terminate.

Rev 9 also said why it was deferred: *re-opening work whose lineage records a success is a distinct capability with its own hazards*. There are three, and they are what the module is shaped by.

1. **The guards that make an issue finished are the guards in the way.** `run_issue` returns `AlreadySucceeded` without dispatching and `plan_queue` defers on it, both reading the same `best_verdict`. Clearing that success would be wrong twice over — it is deliberately never downgraded (*"best so far"*, not *"most recent"*), and rung 10's `plan_advance` needs it to find the PR again once the fix is pushed. So the success stays, and a **second, narrower reason to dispatch** is stored beside it. Re-opening finished work is then an explicit, recorded act rather than the erasure of the record saying it was finished.
2. **The gate is already green, so the gate cannot be the goal.** This is the one that would have shipped silently. A remediation dispatch renders the same `/goal` condition as any other — *"the approved gate passes"* — and at the reviewed commit it already does: the IC would run the gate, watch it pass, report `met`, push nothing, and the reviewer would read the identical commit and return the identical verdict, forever, at full price per turn. The turn prompt therefore **extends the condition**: the gate must pass *and* the findings must be addressed by a commit pushed to the branch. It also states, in the IC's own terms, that the gate is already green — so the instruction cannot be satisfied the easy way by accident.
3. **Every axis it can spend on was already bounded, and none of them was widened.** Dispatches are bounded by the per-issue attempt cap; reviews by rung 10's head-SHA trigger, so a stuck remediation is never re-billed for a second opinion on a diff that has not changed; the day by rung 5's ledger. Rung 11 adds a new *reason* to spend, not a new kind of spending.

Four decisions in it are load-bearing:

- **A remediation ends when the issue records a newer success than the one it was armed against — not when a flag says so.** There is no `resolved` field. A flag is a write that can be lost and a state that can disagree with the world; a newer `best_commit_sha` is true exactly when the IC pushed a fix that went green. A *failed* remediation dispatch leaves that value untouched, so the remediation stays in force and the next tick tries again — which is the error table's "re-dispatch … counting against the same cap", not a single retry. Rung 10's lesson 46 pointed at a different question: the claim is about a commit.
- **The merge is rehearsed while a remediation is in force**, and that is the mechanism rather than a nicety. Rung 6's real `NeedsChanges` hold sets `agent:blocked`, which strips `agent:ready`, drops the issue out of the listing both the Lead and `advance` are built from, and carries no question marker for `blocked::poll_answer` to recover it by. A re-dispatch armed alongside that hold would never be dispatched.
...
### The pull request nobody opened ⟨r11⟩ — defect 7

Rev 1 wrote *"every IC terminal state is an open PR"*, and the data flow drew *"push feature branch, open PR"* as one arrow out of a green IC. **The second half was never built.** `turn_prompt` never asked an IC for a pull request, or even for a push; `gh.rs` had `pr view`, `pr list` and `pr merge` and no `pr create`; nothing in `autopilot/` shelled `git push`. Eleven rungs consumed the assumption without anyone owning it — `advance.rs`'s own module header asserted the IC "pushes its branch and opens a PR" — and the first live run's pull request (#332) was opened by hand.

It is the worst stall in the subsystem, because it is the only one that cannot be recovered by the automation. `run_issue` refuses to dispatch an issue whose lineage records a success, so the IC cannot be sent back; `advance` then reports `NoOpenPr` on every pass, forever, and only a human unlabelling the issue breaks the cycle.

Two halves close it:

1. **The push is in the goal condition, not in the prose.** A gate runs in the worktree, so an IC that edits, commits and goes green satisfies *"the gate passes"* **completely and truthfully** while the remote branch stays empty. The condition now reads *"…and every commit you have made is pushed to this issue's branch `<branch>`"*. Phrased over the commits the dispatch made rather than over a commit existing, so an issue needing no code change is still a legitimate success rather than one reportable only by pushing an empty commit. The branch is named, so a dispatch that cuts its own cannot satisfy it. The IC is told **not** to open the pull request itself.
2. **`advance` opens it**, in `open_pull_request`, by the move rung 10 made on `AlreadySucceeded`: the permanent dead end becomes a step.

Four decisions in it are load-bearing:

1. **`PrLookup::None` is three situations and only one of them may be answered with a write.** Nothing was ever opened; a human closed one; one merged and the issue stayed open. A second `gh pr list --state all` read separates them, and **any** prior pull request — open, closed or merged — stops the create. Re-opening a PR a human closed would overrule that human with a cron entry every few minutes; opening a second for merged work would propose an empty diff forever. The guard runs first, before anything else can fail for an unrelated reason.
2. **The base is read from GitHub, never guessed and never taken from the target.** `advance`'s targets carry `base: "HEAD"`, because the pass cuts no branches — `HEAD` names a commit, and a pull request needs a branch. The base decides what the diff is measured against and therefore what a reviewer reads and what a merge lands, so an unreadable default branch is an error rather than a fallback to `main`.
3. **Opening the pull request is the whole step; the pass does not go on to review it.** Create-then-review-then-merge in one call would make the first pass after a green IC the most expensive thing the subsystem does, against a pull request seconds old. The next pass reviews it through the ordinary lookup.
4. **It is not behind a flag, and `--merge`, `--remediate` and `--advisor` are.** The rule those three follow (rung 9's lesson 43) is about optional features changing what an existing configuration does. This is reversible, spends nothing, re-opens no finished work, and is behaviour this document has specified since rev 1 — the only thing that changes is which component performs it. `--dry-run` is honoured, because that flag means *change nothing* rather than *do less*; the reads still run, so the rehearsal reports the base it would have targeted.

Two situations GitHub reports that are facts rather than failures: a *lost race* (a pull request appeared between the two reads — the next pass finds it) and *no commits between base and head*, which names an IC that recorded a success without pushing. The second is the loud form of the defect above, and replaces a bare "no open PR" an operator could not tell from a pull request they had closed themselves.
             ...
             └─► create git worktree for this issue
                  └─► Lead assigns session uuid; launcher spawns IC DISPATCH
                       ...
                       └─► green
                            ...
                            ├─► IC pushes feature branch   [never pushes a default branch]
                            ├─► Lead opens PR via `gh pr create`  ⟨r11⟩
                            │    [only if no PR ever existed on the branch]
...
⟨r9⟩ **Everything below the `green` branch of this diagram is executed by `autopilot advance`, not by the Lead tick.** Rung 10; see *Who merges*. Before it, the arc was drawn but never run. ⟨r10⟩ **Including the `NEEDS CHANGES ─► re-dispatch IC to fix` arrow**, which rung 10 left as its residual and rung 11 executes under `--remediate`; see *The red path*.
...
| Reviewer uncertain, or returns NEEDS CHANGES | ⟨r2⟩ Re-dispatch the IC to fix, counting against the same per-issue attempt cap. On exhaustion the PR stays open for a human — never merged with an unresolved finding. ⟨r10⟩ **Built, rung 11** (`autopilot/remediate.rs`), behind `autopilot advance --remediate`; without the flag the PR is held for a human immediately, which is rung 10's behaviour. |
