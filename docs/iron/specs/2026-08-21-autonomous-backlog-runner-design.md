...
9. ✅ ⟨r7⟩ **Whether the Lead is a Claude session at all. — CLOSED by rungs 7 and 8: a Rust-native mechanical supervisor, with three isolated one-shot calls.** Rev 6 narrowed this and left it open "pending rung 4/8 implementation experience", naming the two responsibilities that could still tip it back toward a persistent Claude session: **cross-repo prioritization** and **thrash detection**. Both have now been built.
   ...
   | Step | Where it is now | Why it is not mechanical |
   |---|---|---|
   | Dispatch-time risk classification | `lead::class_for` reads a `risk:*` label; absent one it falls back to `unclassified`, which **fails closed** at `decide_merge`'s `ClassMismatch` | Reading an issue's prose to route it |
   | Composing a strategy redirect | Rung 7 generates it mechanically | It can name a repeated failure and forbid repeating it; it cannot propose a better approach |
   | Drafting a human escalation question | `blocked::ask_human` takes the text from its caller | Naming *what* is unclear is the judgment |
   ...
   **The decisive property is not that the three calls turned out to be unnecessary — it is that the loop does not depend on them being available.** A Lead that never makes one still runs: every issue dispatches as `unclassified` and therefore cannot auto-merge, redirects take rung 7's mechanical form, and no questions are asked. That is exactly what a cron-restarted supervisor needs, and it is what a long-lived Claude session — which can wedge, and whose context the watchdog's restart erases — cannot offer. Rung 8 implements the Lead as `lead::lead_tick`: one bounded pass, no resident state, `ironmem autopilot lead`.
   ...
   ⚠️ **Residual, stated not hidden.** The three one-shot calls are *specified and located*, not built — no rung has yet made an LLM call from the supervisor. `--fallback-class unclassified` is the safe stand-in for the first, and the other two are operator-supplied text today.
