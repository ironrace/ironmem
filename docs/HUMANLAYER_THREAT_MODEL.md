...
5. Install a reviewed copy of `scripts/test_humanlayer_workspace_policy.py` at the Host-operator-controlled read-only path `/opt/ironmem-humanlayer/policy/test_humanlayer_workspace_policy.py`. The Host operator verifies every component of `/opt/ironmem-humanlayer/policy/test_humanlayer_workspace_policy.py` with `lstat` or approved equivalent before launch: no symlink, numeric UID 0, approved non-agent-controlled platform group, and no pilot/group/other write permission. The agent-writable repository copy is CI source only and is never directly trusted or executed as the launch gate.
...
The **Host operator** performs an atomic, privileged installation of the reviewed helper at `/opt/ironmem-humanlayer/policy/test_humanlayer_workspace_policy.py`. Stage reviewed bytes in the same root-controlled policy directory, set UID 0, the approved non-agent-controlled platform group, and a non-writable mode, fsync the file and directory where the platform supports it, verify SHA-256, then atomically rename into place. The agent and pilot users must never write the staged file, leaf, or any parent. On Darwin/macOS the default approved ownership is `root:wheel` (numeric UID 0, GID 0); on Linux it is `root:root` (numeric UID 0, GID 0). An approved non-agent-controlled system group may be used only when its numeric GID and resolved name are recorded in the approved manifest.

Before every launch, use trusted host-side `lstat` or equivalent—not an agent-provided path—to inspect `/opt`, `/opt/ironmem-humanlayer`, `/opt/ironmem-humanlayer/policy`, and the helper leaf. Each component must be a non-symlink, have numeric UID 0, the approved platform GID, and no pilot/group/other write permission. Record numeric UID/GID/mode and resolved owner/group names for **every** component, plus the helper SHA-256, and compare to the approved manifest. The helper validates the exact Git worktree root and itself invokes `/usr/bin/git`; the agent-writable repository copy remains CI source only, never a direct launch gate.

The trusted sanitized launch-gate chain is platform-specific. On Darwin, first resolve targets under the selected approved absolute Developer directory with `/usr/bin/env -i PATH=/usr/bin:/bin DEVELOPER_DIR=<approved-absolute-developer-directory> /usr/bin/xcrun --find python3` and the same command ending `--find git`; record the selected Developer directory, reviewed `/usr/bin/xcrun` absolute path/SHA-256, and each resolved target absolute path/SHA-256. Then invoke exactly `/usr/bin/env -i PATH=/usr/bin:/bin DEVELOPER_DIR=<approved-absolute-developer-directory> /usr/bin/python3 -I -S /opt/ironmem-humanlayer/policy/test_humanlayer_workspace_policy.py --repo-root <absolute-pilot-worktree>`. `/usr/bin/python3` and `/usr/bin/git` can be `xcrun` dispatchers on Darwin, so dispatcher hashes alone do not pin the executed targets. On Linux, verify that `/usr/bin/python3` and `/usr/bin/git` are the approved real binaries and record their absolute paths/SHA-256; invoke exactly `/usr/bin/env -i PATH=/usr/bin:/bin /usr/bin/python3 -I -S /opt/ironmem-humanlayer/policy/test_humanlayer_workspace_policy.py --repo-root <absolute-pilot-worktree>`.

Use a fixed minimal system path and absolute reviewed executables for the agent, for example `/usr/bin/env -i PATH=/usr/bin:/bin HOME=/pilot/home LANG=C /pilot/approved/bin/humanlayer <approved-subcommand>`; the configured provider executable must likewise be an approved absolute path, never a `PATH` lookup. The **Host operator** owns host toolchain, Developer-directory, target-resolution, helper, and full parent-chain evidence/remediation. The **Security reviewer** owns HumanLayer executable/version/integrity evidence and manifest approval/remediation. The **Model-provider administrator** owns provider executable/account evidence and remediation. Any approved-manifest change or resumption after a host, HumanLayer, or provider drift needs Security reviewer approval. A mismatch, `@latest`, floating version, or unattended update is **BLOCKED**.
...
| Trusted host toolchain | Host operator | `/usr/bin/env` path/SHA-256; on Darwin, selected absolute Developer directory, `/usr/bin/xcrun` path/SHA-256, and sanitized `xcrun --find python3`/`--find git` target paths/SHA-256; on Linux, approved real `/usr/bin/python3` and `/usr/bin/git` paths/SHA-256. Verify before every launch. |
| Immutable workspace-policy helper | Host operator | Reviewed helper at `/opt/ironmem-humanlayer/policy/test_humanlayer_workspace_policy.py`, outside the agent-writable worktree. Approved manifest records helper SHA-256 and, for `/opt`, `/opt/ironmem-humanlayer`, `/opt/ironmem-humanlayer/policy`, and the leaf, `lstat` non-symlink state, numeric UID/GID/mode, and resolved owner/group names. UID must be 0, group must be approved and non-agent-controlled, and no component may be pilot/group/other writable. Installation is privileged atomic stage/fsync/verify/rename; repository copy is CI source only. |
...
| Host toolchain, Developer directory, `/usr/bin/xcrun`, Darwin-resolved Python/Git target, Linux real Python/Git binary, immutable-helper parent chain/leaf path/hash/UID/GID/mode, or symlink state drifts | **Host operator** stops; do not launch; preserve safe path/hash/numeric-owner/group/mode/target-resolution evidence; restore or review approved host artifacts; **Security reviewer** approves any manifest change and resumption. |
...
- [ ] Host operator: before every daemon/agent launch, `lstat` or equivalent-check `/opt`, `/opt/ironmem-humanlayer`, `/opt/ironmem-humanlayer/policy`, and the immutable helper leaf for non-symlink state, numeric UID 0, approved non-agent-controlled platform GID, no pilot/group/other write bit, and recorded numeric UID/GID/mode plus resolved names; compare helper SHA-256 and the full parent chain to the approved manifest. The repository copy is CI source only, not trusted directly at launch; evidence: ______
- [ ] Host operator: before every launch, verify trusted host toolchain evidence. On Darwin record selected absolute `DEVELOPER_DIR`, `/usr/bin/xcrun` path/SHA-256, and sanitized `xcrun --find python3`/`--find git` target paths/SHA-256; on Linux record approved real `/usr/bin/python3` and `/usr/bin/git` paths/SHA-256; on both record `/usr/bin/env` path/SHA-256. Execute the matching exact helper chain with `--repo-root <absolute-pilot-worktree>` and record selected repo root, `Ran 2 tests`, and `OK`; evidence: ______
- [ ] Security reviewer: before every launch, record approved HumanLayer executable absolute path, version, package integrity, and runtime SHA-256; compare to the approved manifest; evidence: ______
...
The **Pilot owner** authorizes only the narrow pilot and owns out-of-band action confirmation. The **Host operator** owns host/env/IronMem isolation, the immutable-helper parent chain, host toolchain, Darwin Developer-directory/dispatcher/target evidence or Linux real-binary evidence, and host remediation. The **GitHub administrator** owns app install, permissions, and drift. The **Security reviewer** owns HumanLayer executable/version/integrity evidence, approves manifest changes and resumptions, and gates classification. The **Vendor owner** obtains HumanLayer controls/contract evidence. The **Model-provider administrator** owns provider executable/account evidence and provider remediation; Security reviewer approval remains required for related manifest changes or resumption.
