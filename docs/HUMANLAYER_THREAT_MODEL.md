...
`LAUNCH-TRANSACTION-MISSING` is also a current **BLOCKED** condition. A future reviewed supervisor must perform one launch transaction: create a nonce and exact immutable `envp` buffer; enter a launch-specific Linux mount namespace/immutable filesystem view; expose `.humanlayer/workspace.json` and `.gitignore` only as root-owned, read-only, validated objects; make `.humanlayer/workspace.local.json` uncreatable and unreadable as an override; and, if task sync requires writes, mount only `.humanlayer/tasks` as a separately writable subtree. It must block if HumanLayer needs any other policy-directory write. The static helper, directly executed without a shell or intermediary, validates inside that exact namespace using open canonical repository and policy descriptors. Its bounded pass record binds the nonce, allowlist version, value-free `envp` digest, mount/namespace identity, file identities/hashes, repository identity, helper identity, **supervisor binary identity/runtime hash/signature**, exact **supervisor configuration identity/content hash/signature/schema/version**, **invocation-policy schema/version/hash and actual argv/cwd/fd/process attributes**, and HumanLayer artifact identity.

Before HumanLayer fd-exec, the privileged supervisor makes an irreversible transition: clear all supplementary groups; set all real/effective/saved GID to the dedicated pilot GID; then set all real/effective/saved UID to the dedicated non-admin pilot UID; clear and lock ambient, effective, permitted, and inheritable capabilities plus securebits; set `no_new_privs`; apply approved umask, rlimits, namespaces, cgroup/network, and seccomp/Landlock/LSM profiles; and close every descriptor except the exact approved descriptor map with its prescribed access mode and `CLOEXEC` state. In the child context, before exec, it verifies UID/GID/groups, capability sets, securebits, `no_new_privs`, descriptor table, and profile identities. It binds those post-drop values to the nonce; any root identity, privilege-regain path, retained supplementary group/capability, unexpected descriptor, or weakened profile is **BLOCKED**.

The supervisor retains the exact `envp` and namespace, performs a descriptor/identity/invocation final recheck immediately before fd-exec with no intervening agent window, then directly executes verified HumanLayer by already-open verified descriptor (or `fd`-exec equivalent) in the **same** namespace with the **same** `envp`, pilot UID/GID/groups, and restrictions. HumanLayer, immutable broker, and provider must execute or inherit that same unprivileged identity and restrictions; the broker cannot regain privilege. Any inability to preserve the binding, any drift, or an unbound helper record is **BLOCKED**. The value-free `envp` digest is usable as binding evidence only because the contract forbids environment secrets. Pilot credentials use only a separately reviewed dedicated credential store, file-descriptor, or provider-auth mechanism; any configuration requiring a HumanLayer/provider environment secret is **BLOCKED**.

HumanLayer normally launches a separate provider agent; the supervisor must not be described as directly launching it. `PROVIDER-SPAWN-MEDIATION-MISSING` is a **BLOCKED** condition for every separate Codex, Claude, or provider executable. A future additive-code broker must be separately approved and configured/proven as the only absolute, root-owned immutable executable HumanLayer can invoke. Its mapping must bind HumanLayer's absolute broker invocation and the provider's exact/strictly allowlisted invocation under the same invocation-policy schema; it rejects argument, `cwd`, descriptor, identity, credential-channel, namespace, or profile drift. For every launch, the same nonce transaction binds **both** broker and provider canonical absolute paths, versions/channels, runtime SHA-256 values, signatures, numeric UID/GID/modes, full canonical no-symlink nonwritable parent chains, broker configuration/mapping hash/version, and inherited post-drop attributes, all compared to the approved manifest. The broker must validate and open-fd execute that already-verified provider artifact without `PATH`, symlink, or TOCTOU bypass, in the same transaction namespace, exact approved `envp`/credential mechanism, and inherited unprivileged restrictions; HumanLayer must be unable to bypass it. Broker source revision, reproducible build, compiler/dependencies/SBOM, hash, signature, tests, configuration/mapping, and full parent-chain evidence require approval. If this cannot be proven, the separate-provider configuration is **PROHIBITED**. An embedded-provider exception is valid only with verified HumanLayer artifact version/hash/signature/SBOM/component manifest, invocation-policy binding, inherited post-drop restrictions, and proof that no separate provider executable or spawn path exists.
...
The **Host operator** owns the immutable Linux baseline, privileged supervisor, exact `envp`, invocation-policy/post-drop, and launch-transaction evidence, static-helper/broker installation and full-parent-chain evidence, and host remediation. The **Security reviewer** owns supervisor/static-helper/invocation-policy/transaction/credential-contract/broker review and pin approval plus Linux HumanLayer artifact approval/remediation. The **Model-provider administrator** owns provider artifact/account and broker-or-embedded/no-spawn evidence and remediation. Any approved-manifest change or resumption after a host, supervisor, helper, invocation policy, transaction, HumanLayer, broker, credential mechanism, or provider drift needs Security reviewer approval. A mismatch, missing `SUPERVISOR-ARTIFACT-MISSING` resolution, missing `STATIC-HELPER-MISSING` resolution, missing `INVOCATION-POLICY-MISSING` resolution, missing `LAUNCH-TRANSACTION-MISSING` resolution, missing `CREDENTIAL-MECHANISM-MISSING` resolution, missing `PROVIDER-SPAWN-MEDIATION-MISSING` resolution, missing Linux artifact capture, `@latest`, floating version, or unattended update is **BLOCKED**.
...
| HOST-05 | Helper pass is checked in a different namespace, filesystem view, environment, or time window than the HumanLayer exec | Critical | One supervisor-controlled nonce-bound launch transaction; same immutable `envp`, namespace, descriptors, final recheck, and fd-exec | Host operator | Transaction is unimplemented; no current binding proof | BLOCKED: `LAUNCH-TRANSACTION-MISSING` |
...
| Supervisor launch transaction | Host operator (implementation/operation); Security reviewer (design/pin approval) | **Unimplemented `LAUNCH-TRANSACTION-MISSING` blocker.** Source/build/SBOM/tests and record schema prove nonce, exact immutable value-free `envp` digest and allowlist version, mount/namespace identity, open descriptor identities/hashes for repo/policy files, repo/helper/HumanLayer identities, **supervisor binary identity/runtime hash/signature and exact config identity/content hash/signature/schema/version**, invocation-policy schema/version/hash, actual argv/cwd/fd/process attributes, irreversible post-drop UID/GID/groups/capability/securebits/`no_new_privs`/profile state, final recheck, and same-namespace fd-exec. The Host operator opens/executes the supervisor by verified descriptor/fd equivalent before the transaction. The immutable view makes workspace config and `.gitignore` root-owned/read-only, local override uncreatable/unreadable, and permits only a separately mounted `.humanlayer/tasks` writable subtree if needed. |
...
| Broker or provider leaf/full-parent-chain canonical path/version/channel/runtime SHA-256/signature/numeric UID/GID/mode, broker mapping hash/version, invocation argv/cwd/fd/credential channel, inherited post-drop identity/capability/`no_new_privs`/profile, or account evidence drifts, or HumanLayer cannot be proven to invoke only the broker | **Host operator** stops before launch; preserve safe install-chain/mapping/invocation evidence; **Model-provider administrator** remediates provider version/channel/account evidence; **Security reviewer** approves broker/manifest/config changes and resumption. |
...
- [ ] Host operator: verified supervisor constructs the exact approved immutable versioned `envp`, invokes the one static helper against open canonical worktree/policy descriptors in the launch namespace, and records only its bounded value-free pass/fail reason codes. Record nonce, allowlist version, value-free envp digest, namespace/mount identity, file/repo/helper/HumanLayer identities/hashes, supervisor binary identity/runtime hash/signature, supervisor config identity/content hash/signature/schema/version, invocation-policy schema/version/hash, and actual argv/cwd/fd/process attributes; unknown/secret-shaped key, missing supervisor/helper, or unbound record blocks launch; evidence: ______
- [ ] Host operator: launch-specific immutable view makes workspace config and `.gitignore` root-owned/read-only, local override uncreatable/unreadable, and permits only separately mounted `.humanlayer/tasks` writes if task sync needs them; evidence: ______
- [ ] Host operator: before HumanLayer fd-exec, clear supplementary groups; set all real/effective/saved GID to dedicated pilot GID then all real/effective/saved UID to dedicated non-admin pilot UID; clear/lock ambient/effective/permitted/inheritable capabilities and securebits; set `no_new_privs`; apply approved umask/rlimits/namespaces/cgroup/LSM or seccomp/Landlock profiles; close all but exact approved FDs with correct CLOEXEC/access modes; evidence: ______
- [ ] Host operator: in child context, verify and bind to nonce post-drop pilot UID/GID/groups, capability sets, securebits, `no_new_privs`, descriptor table, namespace/network/cgroup/profile identities and hashes. Root, regain path, retained authority, extra FD, or profile drift blocks launch; evidence: ______
- [ ] Host operator: with no intervening agent window, final-recheck exact invocation argv/cwd/envp/FD/process attributes and descriptors, then fd-exec verified HumanLayer in the same namespace with the same `envp` and unprivileged restrictions; evidence: ______
...
|---|---|---|
| Public or synthetic data in one disposable repo on a dedicated approved Linux host/account | BLOCKED (eligible to become PERMITTED) | `STATIC-HELPER-MISSING`, `SUPERVISOR-ARTIFACT-MISSING`, `INVOCATION-POLICY-MISSING`, `LAUNCH-TRANSACTION-MISSING`, and `CREDENTIAL-MECHANISM-MISSING` block every current launch; `PROVIDER-SPAWN-MEDIATION-MISSING` blocks every normal separate provider. It may become PERMITTED only after every checklist item, immutable Linux/supervisor-artifact/static-helper/invocation-policy/transaction/broker gate, dedicated credential mechanism, draft-only workflow, and canary passes. |
...
| Linux-only eligible host/workspace/GitHub/IronMem controls and `STATIC-HELPER-MISSING`/`SUPERVISOR-ARTIFACT-MISSING`/`INVOCATION-POLICY-MISSING`/`LAUNCH-TRANSACTION-MISSING`/`CREDENTIAL-MECHANISM-MISSING`/`PROVIDER-SPAWN-MEDIATION-MISSING` blockers | Executive decision; Pilot architecture; GitHub manifest; credential preflight; copying verification; Supply-chain manifest; Failure handling; Critical blockers |
...
| HumanLayer provider-spawn mediation or embedded/no-spawn proof | System and trust boundaries; Data inventory and flow; Credential and environment preflight; Risk register; Supply-chain manifest; Pilot checklist; Failure handling |
