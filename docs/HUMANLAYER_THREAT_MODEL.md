...
The **Host operator** owns the immutable Linux baseline, privileged supervisor, exact `envp` and launch-transaction evidence, static-helper/broker installation and full-parent-chain evidence, and host remediation. The **Security reviewer** owns static-helper, transaction, credential-contract, and broker review/pin approval plus Linux HumanLayer artifact approval/remediation. The **Model-provider administrator** owns provider artifact/account and broker-or-embedded/no-spawn evidence and remediation. Any approved-manifest change or resumption after a host, helper, transaction, HumanLayer, broker, or provider drift needs Security reviewer approval. A mismatch, missing static helper, missing transaction, missing provider mediation, missing dedicated credential mechanism, missing Linux artifact capture, `@latest`, floating version, or unattended update is **BLOCKED**.
...
| Codex/Claude CLI/provider | Host operator (broker/provider install-chain); Model-provider administrator (provider version/channel/account); Security reviewer (broker/manifest approval) | **Unimplemented `PROVIDER-SPAWN-MEDIATION-MISSING` blocker** for every separate executable. For **both** immutable broker and provider leaf record canonical absolute path, version/channel, runtime SHA-256, signature, numeric UID/GID/mode, every canonical no-symlink nonwritable parent-chain component, and approved-manifest comparison. Record broker source revision/reproducible build/compiler/dependencies/SBOM, tests, broker configuration/mapping hash/version, and signature. HumanLayer must be configured and proven able to invoke only the absolute root-owned immutable broker. Per transaction bind broker+provider+mapping identities, hashes, signatures, and versions; broker opens and fd-execs the already-verified provider in the same bound namespace and exact `envp`/credential mechanism without `PATH`, symlink, or TOCTOU bypass. For an embedded/no-spawn claim instead, capture verified HumanLayer artifact version/hash/signature/SBOM/component manifest and proof that no separate provider executable or spawn path exists; bind those component fields to the transaction. |
...
| Broker or provider leaf/full-parent-chain canonical path/version/channel/runtime SHA-256/signature/numeric UID/GID/mode, broker mapping hash/version, or account evidence drifts, or HumanLayer cannot be proven to invoke only the broker | **Host operator** stops before launch; preserve safe install-chain/mapping evidence; **Model-provider administrator** remediates provider version/channel/account evidence; **Security reviewer** approves broker/manifest/config changes and resumption. |
...
- [ ] Host operator and Security reviewer: approved versioned exact minimal `envp` allowlist, secret-shaped-key rejection with no exception, bounded value-free reason-code schema, and non-environment credential mechanism captured; any HumanLayer/provider environment-secret requirement is BLOCKED; evidence: ______
...
- [ ] Host operator: with no intervening agent window, final-recheck the exact descriptors/identities and directly fd-exec verified HumanLayer in the same namespace with the same `envp`; evidence: ______
...
- [ ] Host operator: for a separate provider, before every launch remeasure broker **and** provider canonical absolute paths, version/channel, runtime SHA-256, signatures, numeric UID/GID/mode, and every canonical no-symlink nonwritable parent-chain component; record broker config/mapping hash/version and compare all fields to the approved manifest; evidence: ______
- [ ] Model-provider administrator: for a separate provider, verify the provider version/channel and account evidence; bind broker+provider+mapping identities/hashes/signatures/versions into the same nonce transaction and confirm the broker opens/fd-execs the already-verified provider and HumanLayer cannot bypass it. For embedded/no-spawn, instead bind verified HumanLayer artifact version/hash/signature/SBOM/component manifest plus no-separate-executable/no-spawn proof; evidence: ______
...
|---|---|---|
| Public or synthetic data in one disposable repo on a dedicated approved Linux host/account | BLOCKED (eligible to become PERMITTED) | `STATIC-HELPER-MISSING`, `SUPERVISOR-ARTIFACT-MISSING`, and `LAUNCH-TRANSACTION-MISSING` block every current launch; `PROVIDER-SPAWN-MEDIATION-MISSING` blocks every normal separate provider, and configurations needing environment credentials are PROHIBITED. It may become PERMITTED only after every checklist item, immutable Linux/supervisor-artifact/static-helper/transaction/broker gate, dedicated credential mechanism, draft-only workflow, and canary passes. |
