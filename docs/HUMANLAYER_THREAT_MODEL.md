...
HumanLayer normally launches a separate provider agent; the supervisor must not be described as directly launching it. `PROVIDER-SPAWN-MEDIATION-MISSING` is a **BLOCKED** condition for every separate Codex, Claude, or provider executable. A future additive-code broker must be separately approved and configured/proven as the only absolute, root-owned immutable executable HumanLayer can invoke. The broker must validate and open-fd execute the approved provider artifact without `PATH`, symlink, or TOCTOU bypass, in the same transaction namespace and exact approved `envp`/credential mechanism; HumanLayer must be unable to bypass it. Its source, reproducible build, SBOM, hash, signature, tests, and full parent-chain evidence require approval. If this cannot be proven, the separate-provider configuration is **PROHIBITED**. An embedded-provider exception is valid only with explicit evidence that the provider is bound to the verified HumanLayer artifact and has no separate spawn path.
...
| Supervisor launch transaction | Host operator (implementation/operation); Security reviewer (design/pin approval) | **Unimplemented `LAUNCH-TRANSACTION-MISSING` blocker.** Source/build/SBOM/tests and record schema prove nonce, exact immutable value-free `envp` digest and allowlist version, mount/namespace identity, open descriptor identities/hashes for repo/policy files, repo/helper/HumanLayer identities, final recheck, and same-namespace fd-exec. The immutable view makes workspace config and `.gitignore` root-owned/read-only, local override uncreatable/unreadable, and permits only a separately mounted `.humanlayer/tasks` writable subtree if needed. |
...
| Codex/Claude CLI/provider | Model-provider administrator (artifact/account); Host operator (host-chain verification); Security reviewer (broker approval) | **Unimplemented `PROVIDER-SPAWN-MEDIATION-MISSING` blocker** for every separate executable. Capture a separately approved immutable absolute broker: source/reproducible build/compiler/dependencies/SBOM, hash, signature, tests, and full parent-chain evidence; HumanLayer must be configured and proven able to invoke only that broker. The broker validates/open-fd-execs the approved provider in the same bound namespace and exact `envp`/credential mechanism without `PATH`, symlink, or TOCTOU bypass. Embedded-provider exception needs explicit binding to the verified HumanLayer artifact and proof of no separate spawn path; otherwise it is not usable. |
...
| `PROVIDER-SPAWN-MEDIATION-MISSING`, broker bypass, unverified provider spawn, or claimed embedded provider without no-spawn proof | **Host operator** stops; separate provider is **PROHIBITED**; preserve safe broker/provider identity evidence; Model-provider administrator remediates and Security reviewer approves any broker/manifest change before resumption. |
...
| Provider executable/broker leaf/full-parent-chain path/hash/signature/version or account evidence drifts, or HumanLayer cannot be proven to invoke only the broker | **Host operator** stops before launch; **Model-provider administrator** preserves safe broker/provider/account evidence and remediates the provider artifact/account; **Security reviewer** approves any manifest change and resumption. |
...
- [ ] Host operator: trusted supervisor constructs the exact approved immutable versioned `envp`, invokes the one static helper against open canonical worktree/policy descriptors in the launch namespace, and records only its bounded value-free pass/fail reason codes. Record nonce, allowlist version, value-free envp digest, namespace/mount identity, file/repo/helper/HumanLayer identities/hashes; unknown/secret-shaped key, missing helper, or unbound record blocks launch; evidence: ______
...
- [ ] Model-provider administrator: before every launch, record approved separate-provider broker path/binding, provider artifact/account evidence, or embedded-HumanLayer/no-spawn proof; confirm HumanLayer cannot bypass the broker and compare to the approved manifest; evidence: ______
...
|---|---|---|
| Public or synthetic data in one disposable repo on a dedicated approved Linux host/account | BLOCKED (eligible to become PERMITTED) | `STATIC-HELPER-MISSING` and `LAUNCH-TRANSACTION-MISSING` block every current launch; `PROVIDER-SPAWN-MEDIATION-MISSING` blocks every normal separate provider, and configurations needing environment credentials are PROHIBITED. It may become PERMITTED only after every checklist item, immutable Linux/supervisor/static-helper/transaction/broker gate, dedicated credential mechanism, draft-only workflow, and canary passes. |
...
Wider rollout remains **BLOCKED** until every item has current evidence and accountable approval: product-specific vendor retention/training/subprocessor/deletion/encryption/isolation/audit/incident/DPA-assurance evidence; demonstrated effective sandbox/approval containment; subtractive-copy support or equivalent independently verified containment; `STATIC-HELPER-MISSING` resolved through a separately approved additive-code issue with reviewed source/reproducible static build/compiler/dependencies/SBOM/test vectors, exact versioned minimal environment contract, pinned signed artifact, durable atomic installation, full immutable parent-chain evidence, and trusted-supervisor direct execution; `LAUNCH-TRANSACTION-MISSING` resolved through a reviewed nonce/envp/namespace/open-descriptor/final-recheck/same-namespace-fd-exec implementation; dedicated non-environment credential mechanism for every credentialed configuration; `PROVIDER-SPAWN-MEDIATION-MISSING` resolved with embedded/no-spawn proof or an approved immutable broker HumanLayer alone invokes; immutable/measured Linux image/supervisor baseline; approved actual Linux HumanLayer artifact and provider artifact/account evidence; one-repo GitHub enforcement and drift response; IronMem isolation canary and disabled optional egress; and tested incident stop/revoke/rotate/delete-request/preserve-evidence handling. The Python policy script, pytest bridge, explicit-root checks, and shell block are CI/development-only and cannot satisfy this gate. Darwin/Xcode dispatcher resolution, target and Developer-directory parent-chain immutability, and TOCTOU-resistant execution remain unresolved, so wider host support is **BLOCKED**.
...
| Linux-only eligible host/workspace/GitHub/IronMem controls and all-launch static-helper/transaction/provider-broker blockers | Executive decision; Pilot architecture; GitHub manifest; credential preflight; copying verification; Supply-chain manifest; Failure handling; Critical blockers |
...
The **Pilot owner** may authorize only an eventually compliant narrow Linux public/synthetic pilot and owns out-of-band confirmation for explicitly allowed actions; no current launch is authorized. The **Host operator** owns the immutable/measured Linux baseline, privileged supervisor, exact `envp`/launch-transaction construction and evidence, static-helper installation/full-parent-chain evidence, HumanLayer/provider/broker host-chain evidence, Linux platform qualification, IronMem isolation, and host remediation; it owns operational remediation of `STATIC-HELPER-MISSING` and `LAUNCH-TRANSACTION-MISSING`. Darwin dispatcher/Xcode facts are blocker rationale, not a Host-operator implementation duty for this pilot. The **GitHub administrator** owns app install, permissions, and drift. The **Security reviewer** owns static-helper and transaction source/build/pin approval, approved environment/credential contract, Linux HumanLayer executable/version/integrity evidence, approves manifest changes and resumptions, and gates classification. The **Vendor owner** obtains HumanLayer controls/contract evidence. The **Model-provider administrator** owns provider artifact/account and broker/embedded-no-spawn evidence and remediation, including `PROVIDER-SPAWN-MEDIATION-MISSING`; Security reviewer approval remains required for related manifest changes or resumption.
